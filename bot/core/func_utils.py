...
async def sendMessage(chat, text, buttons=None, get_error=False, **kwargs):
    try:
        # If chat is an integer (channel or user ID)
...
async def get_fsubs(uid, txtargs):
    txt = "<b><i>Pʟᴇᴀsᴇ Jᴏɪɴ Fᴏʟʟᴏᴡɪɴɢ Cʜᴀɴɴᴇʟs ᴛᴏ Usᴇ ᴛʜɪs Bᴏᴛ!</i></b>\n\n"
    btns = []
    ...
    if len(txtargs) > 1:
        btns.append([InlineKeyboardButton('🗂 Tʀʏ Aɢᴀɪɴ', url=f'https://t.me/{(await bot.get_me()).username}?start={txtargs[1]}')])
    return txt, btns
