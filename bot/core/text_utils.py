...
class TextEditor:
    ...
    async def get_upname(self, qual=""):
        ...
        lang = 'Dual Audio' if 'dual-audio' in self.__name.lower() else 'Sub'
        ...
        if anime_name and self.pdata.get("episode_number"):
            ...
            return f"""[S{anime_season}-{str(self.pdata.get('episode_number')) if self.pdata.get('episode_number') else ''}] {titles.get('english') or titles.get('romaji') or titles.get('native')} {'['+qual+'p]' if qual else ''} {'['+codec.upper()+'] ' if codec else ''}{'['+lang+']'} {Var.BRAND_UNAME}.mkv"""
    ...
    async def get_caption(self):
        ...
        return CAPTION_FORMAT.format(
                ...
                genres=", ".join(f"{x.replace(' ', ' ').replace('-', '-')}" for x in (self.adata.get('genres') or [])),
                ...
                lang = 'English & Japanese' if 'dual-audio' in self.__name.lower() else 'Japanese',
